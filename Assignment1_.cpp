...
//hashing using linear probing with chaining with replacement
struct rec{
    int rolln;
    int chain;
};
...
class Hashing{
    ...
    void insert(int r){
        ...
        if(hashtable[pos].rolln==-1){
            ...
            ct++;
        }else if(hashtable[pos].rolln%10!=pos){
            int cr=hashtable[pos].rolln;
            hashtable[pos].rolln=r;
            pos=cr%10;
            p=cr%10;
            while(true){
                    pos++;
                    pos=pos%10;
                    if(hashtable[pos].rolln%10==cr%10){
                        p=pos;
                        }
                        if(hashtable[pos].rolln==-1){
                        hashtable[pos].rolln=cr;
